...
# ----------------------------------------------------------------------
# CORRECT CONFIDENCE ELLIPSE FUNCTION
# ----------------------------------------------------------------------
def confidence_ellipse(x, y, ax, n_std=1.0, facecolor='none', **kwargs):
    """
    Draw a confidence ellipse using the covariance eigen-decomposition.
    """
    if x.size != y.size:
    ...
    cov = np.cov(x, y)
    debug("Covariance matrix:", cov)

    # Eigen-decomposition
    eigenvals, eigenvecs = np.linalg.eigh(cov)
    debug("Eigenvalues (unsorted):", eigenvals)
    debug("Eigenvectors (unsorted):", eigenvecs)
    ...
    debug("Eigenvalues (sorted):", eigenvals)
    debug("Eigenvectors (sorted):", eigenvecs)

    # Width & height = 2σ along principal axes
    width = 2 * n_std * np.sqrt(eigenvals[0])
    height = 2 * n_std * np.sqrt(eigenvals[1])

    debug("Ellipse width:", width)
    debug("Ellipse height:", height)

    # Orientation angle
    angle = np.degrees(np.arctan2(eigenvecs[1, 0], eigenvecs[0, 0]))
    debug("Ellipse angle:", angle)
...
def main():
    ...
    for i, (filepath, label) in enumerate(files_labels):
        ...
        # ------------------------------------------------------------------
        # Plot raw points for each scheme
        # ------------------------------------------------------------------
        ax.scatter(
            delays, throughputs,
